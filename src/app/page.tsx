...
const CAT_BADGE: Record<string, string> = {
  union: "bg-blue-100 text-blue-700", employer: "bg-green-100 text-green-700",
  government: "bg-amber-100 text-amber-700", court: "bg-red-100 text-red-700",
};
const CAT_LABEL: Record<string, string> = {
  union: "👥 노조", employer: "🏭 사용자", government: "🏛️ 정부", court: "⚖️ 법원",
};

const QUICK_BUTTONS: { label: string; query: string; mode: Mode }[] = [
  { label: "📰 오늘 브리핑", query: "오늘의 노동뉴스 브리핑해줘",       mode: "daily" },
  { label: "📋 주간 브리핑", query: "이번 주 주요 노동뉴스 브리핑해줘", mode: "weekly" },
  { label: "👥 노조",        query: "오늘 노조 관련 뉴스만 알려줘",      mode: "daily" },
  { label: "🏛️ 정부",       query: "오늘 정부 노동정책 뉴스 알려줘",    mode: "daily" },
  { label: "⚖️ 법원",       query: "오늘 법원 노동 판결 뉴스 알려줘",   mode: "daily" },
];
...
const WELCOME = "오늘의 주요 노동뉴스를 노조·사용자·정부·법원으로 분류해 요약과 원문 링크와 함께 제공합니다.\n📰 오늘 브리핑 또는 📋 주간 브리핑 버튼을 눌러 시작하세요.";

export default function Home() {
  const [messages, setMessages] = useState<Message[]>([
    { id: "welcome", role: "assistant", raw: "", parsed: { type: "text", content: WELCOME } },
  ]);
  ...
  async function send(text: string, mode: Mode = "daily") {
    ...
    const apiMsgs = messages
      .filter((m) => m.id !== "welcome" && !m.isLoading && m.raw)
    ...
    try {
      ...
      const res = await fetch(`${base}/api/chat`, {
        ...
        body: JSON.stringify({ messages: [...apiMsgs, { role: "user", content: text }], mode }),
      });
      ...
      <div className="w-full max-w-2xl px-4 pt-1">
        <div className="flex flex-wrap gap-2">
          {QUICK_BUTTONS.map((btn) => (
            <button key={btn.label} onClick={() => send(btn.query, btn.mode)} disabled={loading}
      ...
      <div className="w-full max-w-2xl px-4 pt-2 pb-6">
        <div className="bg-white border border-gray-200 rounded-2xl flex items-center gap-2 px-4 py-2.5 shadow-sm">
          <input ref={inputRef} type="text" value={input}
            ...
            onKeyDown={(e) => e.key === "Enter" && send(input, "daily")}
            placeholder="노동뉴스에 대해 질문하세요..."
            disabled={loading}
            className="flex-1 text-sm text-gray-800 placeholder-gray-400 outline-none bg-transparent" />
          <button onClick={() => send(input, "daily")} disabled={loading || !input.trim()}
